...
- **멀티 탭**: 여러 문서를 탭으로 열어서 전환하며 보기 (더블클릭으로 탭 이름 수정, 마우스 오버 시 소스 경로 표시)
- **한국어 번역**: 영문 마크다운을 한국어로 번역 (원본 보기 지원)
- **다크 모드**: 라이트/다크 테마 전환 (설정 자동 저장)
- **목차(TOC)**: 헤딩 기반 자동 생성, 클릭 시 해당 섹션으로 스크롤
- **TOC 사이드바 조절**: 드래그로 너비 조절, 숨기기/표시 토글

## 사용법

1. **파일 열기**: "Open File" 버튼으로 로컬 .md 파일 선택 (새 탭으로 열림)
2. **URL 로드**: URL 입력 후 "Load" 버튼 클릭
3. **붙여넣기**: "Paste" 버튼 클릭 후 마크다운 내용 붙여넣기
4. **탭 전환**: 상단 탭 바에서 문서 간 전환, × 버튼으로 탭 닫기, 더블클릭으로 탭 이름 수정
5. **번역**: "Translate to KO" 버튼으로 한국어 번역
6. **원본 보기**: 번역 후 "Show Original" 버튼으로 원본 복원
7. **TOC 조절**: 사이드바 가장자리 드래그로 너비 조절, ◀/▶ 버튼으로 숨기기/표시
